...
<?php else: ?>
    ...
    <?php if ($project['is_private'] == 0 && ! empty($users['not_allowed'])): ?>
        <hr/>
        <form method="post" role="form" class="form-inline" action="<?= $this->url->href('project', 'allow', array('project_id' => $project['id'])) ?>" autocomplete="off">
            ...
            <div class="form-actions">
                <button type="submit" value="<?= t('Allow this user') ?>" class="btn btn-primary">Allow this user</button>
            </div>
        </form>
    <?php endif ?>
...
<?php if ($project['is_private'] == 0): ?>
<hr/>
<form method="post" role="form" class="form-inline" action="<?= $this->url->href('project', 'allowEverybody', array('project_id' => $project['id'])) ?>">
    <?= $this->form->csrf() ?>
